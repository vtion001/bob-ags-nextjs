...
interface NavItem {
  ...
  icon: string
  description: string
  permission: keyof Permissions | 'settings' | 'none'
}
...
export default function DashboardLayout({
  ...
  useEffect(() => {
    const checkSession = async () => {
      try {
        ...
        const permRes = await fetch('/api/users/permissions')
        if (permRes.ok) {
  ...
  const getFilteredNavItems = (): NavItem[] => {
    return allNavItems.filter(item => {
      if (item.permission === 'settings') {
        return permissions.can_manage_settings || role === 'admin'
      }
      if (item.permission === 'can_view_calls') {
        return permissions.can_view_calls
      }
      if (item.permission === 'can_view_monitor') {
        return permissions.can_view_monitor
      }
      if (item.permission === 'can_view_history') {
        return permissions.can_view_history
      }
      if (item.permission === 'can_view_agents') {
        return permissions.can_view_agents
      }
      return true
    })
  }
  ...
  const renderIcon = (icon: string, isActive: boolean) => {
    const activeClass = isActive ? 'bg-navy-900 text-white' : 'bg-white/10 text-white/80 group-hover:bg-white/20 group-hover:text-white'
    
    return (
      <span className={`flex-shrink-0 w-9 h-9 rounded-lg flex items-center justify-center transition-colors ${activeClass}`}>
        {icon === 'phone' && (
                ...
                <Link key={item.href} href={item.href}>
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <span className={`text-xs block truncate ${isActive ? 'text-navy-500' : 'text-white/50'}`}>
                        {item.description}
                      </span>
                    </div>
                  </span>
          ...
          <div className="p-4 m-3 rounded-xl bg-white/10 border border-white/20">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <span className="text-xs text-white/60">{role.charAt(0).toUpperCase() + role.slice(1)}</span>
              </div>
            </div>
          </div>
        </aside>
